---
title: "Analysis Template with Example Data"
...
fill_color <- "#ADF097" #define color for fill
...
If your data is not normally distributed, you may need to transform it to meet the assumptions of some statistical tests. Below are examples of some common transformations you might use for ecological data. Note that it's better to test residuals when running many statistical models before deciding to use transformed data, and in some cases you may apply a different statistical method that doesn't have the same assumptions. If you do find your data doesn't match the assumptions of your analysis needs though, these can be useful transformations.
...
# Box plots and bar charts of variables of interest

Now that we have a feel for the data, it can be helpful to start by looking at basic plots to show the means and variance of key response variables across a factor of interest, such as site or treatment. Below are examples of two of the most useful plot types for this purpose: box plots and bar charts.
...
If you have multiple response variables or are interested in the relationships between variables, it can be difficult to know where to start. Below are a few common analyses that can help you get a better understanding of relationships within your data.

## Correlations

Correlations between variables can be a nice place to start. This can help you identify relationships between variables and determine which variables are good candidates for further analysis.
...
Unconstrained ordination methods are useful for visualizing the relationships between variables in a dataset. They can help you identify patterns in your data and determine which variables are most important for explaining the variation in your dataset.

### PCA (Principal Components Analysis)

Principal Components Analysis (PCA) is a common method for reducing the dimensionality of a dataset and visualizing the relationships between variables. It can help you identify patterns in your data and determine which variables are most important for explaining the variation in your dataset.

```{r pca, fig.align = "center", out.width='100%', fig.cap="PCA biplot of variables."}

#select only columns with a value in every row, i.e. no NA values
df_comp_std <- df_std[ , colSums(is.na(df_std)) == 0]
df_comp_std #view selected data

#PCA with base R
pca <- prcomp(df_comp_std) #perform PCA on standardized variables - depending on the data, you may want to use the transformed and standardized data instead of just standardized data

summary(pca) #view importance of principal components and proportion of variances
pca$rotation #view loadings from the standardized data
screeplot(pca) #diagnostic plot to make sure principal components are in order

#Create biplot of PCA results
autoplot(pca, data = df_comp_std, #plot pca from pca model and data
				 colour = 'Site', #choose variable to separate by, such as site or species
         loadings = TRUE, loadings.colour = '#C8AF67', #show loadings, define color
         loadings.label = TRUE, loadings.label.size = 3, 
				 loadings.label.colour = 'black',
				 #label = TRUE, label.size = 3, #show labels for datapoints
) + my_theme + #custom theme from Chunk 3
  ggtitle("PCA Biplot") #add plot title

#save figure - name, type, folder, dimensions, resolution, background specs
ggsave('data_pca.svg', path = "Figures", width = 6, height = 4, units = "in", dpi = 300, bg=NULL)

```

### NMDS (Nonmetric Multidimensional Scaling)

Nonmetric Multidimensional Scaling (NMDS) is another method for visualizing the relationships between variables in a dataset. It is particularly useful for ecological data, where the relationships between variables may not be linear and data may not meet the assumptions of a PCA. NMDS can help you identify patterns in your data and determine which variables are most important for explaining the variation in your dataset. However, it's important to note that NMDS may not be as interpreted in the same way as PCA.

```{r nmds, fig.cap="Nonmetric multidimensional scaling (NMDS) plot, using the Bray-Curtis dissimilarity coefficient."}

#create distance matrix with Bray-Curtis dissimilarity using the vegan package
df_dist <- vegdist(dataframe, method = "bray", na.rm = TRUE) #change method to other dissimilarity measures as needed
df_dist <- as.matrix(df_dist, labels = TRUE) #convert to matrix
df_dist #view distance matrix

#check for complete rows - if not enough, might consider removing columns with too many NAs
df_completerows <- df_std[complete.cases(df_std), ] #check how many complete rows left for envfit
df_completerows #view complete rows

df_nmds <- df_std[,-c(1:2,5)] #remove categorical variables and variables with too many NAs - change column numbers as needed

#NMDS with vegan package
#test how many axes are appropriate
stress <- vector(length = 6)
for (i in 1:6) {
    stress[i] <- metaMDS(df_nmds, distance = "bray", k = i,
...
#run NMDS with 3 axes - change number of axes as needed
set.seed(42) #set seed for reproducibility
nmds <- metaMDS(df_nmds, distance = "bray", #data & distance measure
                k = 3, #number of axes
                model = "global", #calc stress on ranks of data
                maxit = 400, #max iterations of configuration adjustments
...
nmds #check "stress" value to see how well the data is represented by the ordination
goodness(nmds) #check goodness of fit for each point
stressplot(nmds, main = "Shepard plot") #diagnostic plot

#make fit vectors and view
nmdsfit <- envfit(nmds, df_nmds, permutations = 999, na.rm=TRUE)
nmdsfit #check fit vectors

#plot with base R
{ordiplot(nmds, type = "n")
orditorp(nmds, display = "sites", labels = F, pch = c(16))
plot(nmdsfit, p.max = 0.054, col = "black", cex = 0.7)} # change the significance level of species shown with p.max

#make df of scores
nmds_scores <- as.data.frame(scores(nmds))
nmds_scores$cat_variable = df_std$cat_variable #add back categorical variable for coloring

#make df of vectors, only significant vectors
nmds_vectors <- as.data.frame(scores(nmdsfit, display = "vectors")) #extracts relevant scores from envifit
nmds_vectors <- cbind(nmds_vectors, variable = rownames(nmds_vectors)) #and then gives them their names
nmds_vectors <- cbind(nmds_vectors, pval = nmdsfit$vectors$pvals) # add pvalues to dataframe
sig_vectors <- subset(nmds_vectors, pval<=0.054) #subset data to show variables significant at rounded 0.05

#plot scores with ggplot
ggplot(nmds_scores) + 
  geom_point(size = 2, aes(x = NMDS1, y = NMDS2, color = cat_variable)) + #add points and color by variable
  geom_segment(data = sig_vectors,  #add vector arrows of significant variables
              aes(x = 0, xend = NMDS1 * 400, #add scaling factor
                  y = 0, yend = NMDS2 * 400), 
              arrow = arrow(length = unit(0.25,"cm"), #add arrow head, spec. length
                            type = "open", angle = 20, #type and angle of arrowhead
                            ends = "last"), #which end to draw arrow head
              color = "grey10", 
              lwd = 0.3) +
  geom_text_repel(data = sig_vectors, #add labels for variables that don't overlap
                  aes(x = NMDS1 * 400, #add scaling factor
                      y = NMDS2 * 400, 
                      label = variable), 
                  size = 3,
                  max.overlaps = Inf) +
  labs(x = "NMDS1", y = "NMDS2", color = "variable1") +
  coord_fixed() + #ensures axes are on the same scale
  my_theme #custom theme from Chunk 3

#save figure - name, type, folder, dimensions, resolution, background specs
ggsave('NMDS_date.svg', path = "Figures", width = 6, height = 4, units = "in", dpi = 300, bg=NULL)
  
```

# Analyze specific relationships

Now that you have a better understanding of the relationships between variables in your dataset, you can start to analyze specific relationships in more detail. Below are examples of common analyses you might use to test specific hypotheses or explore relationships between variables.

## Linear Regression, Linear Mixed Models, Generalized Linear Models

Linear regression is a common method for analyzing the relationship between a continuous response variable and one or more predictor variables. It can help you determine the strength and direction of the relationship between variables and make predictions based on the model. Additionally, group-wise comparisons such as analysis of variance (ANOVA), discussed further below, depend on linear regression.
...
Generalized linear models (GLMs) are an extension of linear regression that can be used for non-normal response variables, such as binary, count, or proportional data. They allow you to model the relationship between a response variable and one or more predictor variables using a variety of link functions.

```{r lm, warning = FALSE}

#Linear regression with base R
lm_model <- lm(response ~ predictor1 + predictor2, data = dataframe) #model with 2 predictor variables

#Linear mixed model with lme4 library, setting a random variable such as site or block
lme_model <- lmer(response ~ predictor1 * predictor2 + (1|randomvariable), #model with 2 interacting predictor variables and 1 random variable
...
#Generalized linear model with lme4 library, setting a link function such as binomial or poisson
glm_model <- glmer(response ~ predictor1 + predictor2 + (1|randomvariable), #model with 2 predictor variables and 1 random variable
    family = binomial(link = "logit"), #binomial link function
    data = dataframe, #choose dataframe
    na.action = na.omit) #omit observations with NA values

```

## Model checks and fit for regression models

After fitting a regression model, it's important to check the assumptions of the model and assess the fit of the model to the data. Below are some common checks you might perform to evaluate the fit of your model. These include checking the normality of residuals, multicollinearity, and model assumptions. You can also use the performance library to check the assumptions of your model and assess the fit of the model to the data.

```{r model checks, warning = FALSE}

#Summary including residual quantiles, variance explained by random effects (in mixed model), fixed effects, and correlation of fixed effects
summary(lm_model) #view summary of linear regression model

#Plot residuals with base R
qqnorm(resid(lm_model), main = "Normal Q-Q Plot", 
			 xlab = "Theoretical Quantiles", ylab = "Sample Quantiles",
			 plot.it = TRUE, col = "darkgrey")
qqline(resid(lm_model), distribution = qnorm,
       probs = c(0.25, 0.75), 
			 col = "dodgerblue", lwd = 2) #create line for normal distribution

#Check assumptions and fit of linear regression model (regular, mixed model or generalized model) with performance library and MuMIn library
shapiro.test(resid(lm_model)) #check normality of residuals
check_collinearity(lm_model) #calculate variance inflation factors (VIF) to check for multicollinearity
performance::check_model(lm_model) #plot model checks
performance::r2(lm_model, multivariate=TRUE) #calculate R-squared for model fit
AIC(lm_model) #calculate AIC for model comparison
MuMIn::AICc(lm_model, k=2) #calculate AICc for for model comparison with small sample sizes
...
A non-parametric post-hoc alternative is the Dunn test, which is used in place of Tukey's HSD when the assumption of homogeneity of variance is violated.
...
#Dunn test with dunn.test library
dunn.test(dataframe$response, dataframe$predictor, method = "bonferroni") #Dunn test with Bonferroni correction

```

## Plot linear regression

```{r lm plot, warning = FALSE, fig.height = 5, fig.width = 3.5, fig.cap="Linear model of response variable by predictor variable."}

#Plot linear regression with ggplot
ggplot(data = dataframe, aes(x=predictor, y=response)) + #define x and y variables
  geom_point(color="grey10") + #add data points
  geom_smooth(method="lm", se=TRUE, color=plotline_color, fill=fill_color) + #add linear regression line with confidence interval fill
  xlab("Predictor") + ylab(expression(paste("Response (unit)"))) +
  scale_y_continuous(breaks = pretty(c(0,50), n = 5)) + #change limits and number of tick marks on y axis
  my_theme
...
If you have specific hypotheses you want to test or want to determine the relationship between response variables and predictor variables, you can use constrained ordination methods. These methods allow you to test the significance of predictor variables and determine the strength and direction of the relationship between variables.

## RDA (Redundancy Analysis)

Redundancy Analysis (RDA) is a constrained ordination method that allows you to test the significance of predictor variables and determine the strength and direction of the relationship between variables. It is particularly useful for analyzing the relationship between response variables and predictor variables and can help you identify the variables that are most important for explaining the variation in your dataset.

```{r rda, fig.align = "center", out.width='100%', fig.cap="Redundancy Analysis (RDA) plot of variables."}

#put response variables into dataframe from standardization and best transformation
respvar <- df_bc_std[,c("response1", "response2")] #select response variables

#put predictor variables into dataframe from standardization and best transformation
predvar <- df_bc_std[,-c("response1", "response2", "response3")] #select all but response variables
pred_comp <- predvar[ , colSums(is.na(predvar)) == 0] #select only columns with a value in every row, i.e. no NA values

#RDA full dataset with responses and predictors with vegan library
rda <- rda(respvar ~., #define response matrix
             data=pred_comp, #define predictor matrix
             scale=FALSE, #don't scale variables - transformed ahead of time
             na.action = na.omit) #omit NA values
summary(rda) #view summary of RDA

#forward selection of variables with vegan library
fwd_sel <- ordiR2step(
  rda(respvar ~ 1, data = pred_comp, ##lower model limit 1
...
fwd_sel$call #call reduced formula

#RDA with selected variables from reduced formula
sel_rda <- rda(formula = respvar ~ predictor1 + predictor2 + predictor3 + predictor4, 
               data = pred_comp, na.action = na.omit)
anova.cca(sel_rda, by = "term") #test variables
anova.cca(sel_rda, by = "axis") #test RDA axis significance
RsquareAdj(sel_rda) #test R-squared for model fit

#base R ordination plot
ordiplot(sel_rda, scaling = 2, type = "t") #scaling 2 reflects correlations

#plot scores with ggplot

#make dfs of values from RDA
rda_sum <- summary(sel_rda)
rda_sites  <- as.data.frame(rda_sum$sites[,1:2]) 
rda_sites$Site = df_std$Site #add site back
rda_response  <- as.data.frame(rda_sum$species[,1:2])
rda_predictors  <- as.data.frame(rda_sum$biplot[,1:2])
rda_vectors <- rbind(rda_response, rda_predictors)

ggplot(rda_sites) + 
  geom_point(size = 1, aes(x=RDA1, y=RDA2, colour = Site)) + #add points and color by site
  geom_segment(data=rda_response, #add vector arrows of significant variables
               aes(x=0, xend=RDA1, 
                   y=0, yend=RDA2), 
               arrow = arrow(length = unit(0.25,"cm"), #add arrow head, spec. length
                            type = "open", angle = 20, #type and angle of arrowhead
                ...
                colour = "grey10", 
                lwd = 0.3) + #line width
  geom_text_repel(data=rda_response, 
            aes(x=RDA1, y=RDA2,
                label=rownames(rda_response),
                hjust=0.5*(1-sign(RDA1)),vjust=0.5*(1-sign(RDA2))), 
                colour = "grey10", size=3) +
  geom_segment(data=rda_predictors, #add vector arrows of significant variables
               aes(x=0, xend=RDA1, 
                   y=0, yend=RDA2), 
               arrow = arrow(length = unit(0.25,"cm"), #add arrow head, spec. length
                            type = "open", angle = 20, #type and angle of arrowhead
                ...
                colour = "red", 
                lwd = 0.3) + #line width
  geom_text(data=rda_predictors, 
            aes(x=RDA1, y=RDA2,
                label=rownames(rda_predictors),
                hjust=0.5*(1-sign(RDA1)),vjust=0.5*(1-sign(RDA2))), 
                colour = "red", size=3) +
  labs(x = "RDA1", colour = "Site", y = "RDA2") +
  coord_fixed() + #ensures axes are on the same scale
  my_theme #custom theme from Chunk 3

#save figure - name, type, folder, dimensions, resolution, background specs
ggsave('RDA.svg', path = "Figures", width = 6, height = 4, units = "in", dpi = 300, bg=NULL)

```

## Variation partitioning

Variation partitioning is a useful technique for quantifying the relative importance of different predictor variables in explaining the variation in a response variable. It allows you to determine the unique and shared contributions of each predictor variable to the variation in the response variable(s). Using the reduced RDA formula from above, we can perform variation partitioning to quantify the relative importance of each identified predictor variable in explaining the variation in the response variable.

```{r varpart, warning = FALSE}

#Variation partitioning with vegan library - use variables from RDA
vpa <- varpart(respvar, ~ predictor1, ~ predictor2, ~ predictor3, ~ predictor4, data=pred_comp)
vpa #view variation partitioning results
vpa$part #view partitioning results

#plot variation partitioning results
plot(vpa, Xnames = c("predictor1", "predictor2", "predictor3", "predictor4"), #set names
     digits=1, bg = colors, alpha=100, id.size=1.1) #set significant digits, colors and labels

#save figure - name, type, folder, dimensions, resolution, background specs
Cairo(file="Figures/vpa.svg", type="svg", width = 6, height = 4, units = "in", pointsize=12, bg = "transparent", dpi = 300)
