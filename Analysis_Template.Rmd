...
nmds <- metaMDS(df_std, distance = "bray", #data & distance measure
                k = 3, #number of axes
                model = "global", #calc stress on ranks of data
                maxit = 400, #max iterations of configuration adjustments
...
nmds_scores$Site = df_std$Site #add site back
...
ggplot(nmds_scores) + 
  geom_point(size = 2, aes(x = NMDS1, y = NMDS2, color = variable1)) + #add points and color by variable
...
#RDA full dataset with responses and predictors
...
#Variation partitioning with vegan library
vpa <- varpart(sel_rda, data = pred_comp) #perform variation partitioning
